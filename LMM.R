...
for (y in outcomes) {
  formula <- formula(paste0(y, '~cpeur2 +', MAIN, '+', COVARS, '+ (1|abcd_site_0y)'))
...
res_lmm4<-matrix(, nrow=9)

for (y in outcomes) {
  formula <- formula(paste0(y, '~eaeur1 +', MAIN, '+', COVARS, '+ (1|abcd_site_0y)'))
